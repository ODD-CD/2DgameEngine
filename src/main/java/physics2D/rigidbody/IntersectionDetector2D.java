...
public class IntersectionDetector2D {
    ...
    public static boolean lineAndCircle(Line2D line, Circle circle) {
        ...
        Vector2f mainLine = new Vector2f(line.getEnd()).sub(line.getStart());

        Vector2f circleCenter = circle.getCenter();
        Vector2f centerToStart = new Vector2f(circleCenter).sub(line.getStart());
        // calculates alignment value of centerToStart to mainLine and normalizes it
        float t = centerToStart.dot(mainLine) / mainLine.dot(mainLine);

        if (t < 0.0f || t > 1.0f) {
            return false;
        }

        // Find the closest point from circleCenter to the mainLine
        Vector2f closestPoint = new Vector2f(line.getStart()).add(mainLine.mul(t));
    ...
    public static boolean circleAndBox2D(Circle circle, Box2D box) {
        ...
        JMath.rotate(r, -box.getRigidBody2D().getRotation(), new Vector2f(0,0));
    ...
    public static boolean AABBAndBox2D(AABB b1, Box2D b2) {
        ...
        JMath.rotate(axesToTest[2], b2.getRigidBody2D().getRotation(), new Vector2f());
        JMath.rotate(axesToTest[3], b2.getRigidBody2D().getRotation(), new Vector2f());
        for (int i = 0; i < axesToTest.length; i++) {
            if (!overlapOnAxis(b1, b2, axesToTest[i])) {
                return false;
            }
